...
$to = 'contact@localhost.ro';			// Adresa de e-mail la care va fi trimis mesajul
...
if(isset($_POST['anti_spam']) && isset($_POST['anti_spam1']) && $_POST['anti_spam']==$_POST['anti_spam1']) {
  ...
  if (isset($_POST['nume']) && isset($_POST['email']) && isset($_POST['subiect']) && isset($_POST['mesaj'])) {
	...
	if($gmail===1) {
	  ...
	  else $send = 'Eroare: Serverul nu a putut expedia mesajul prin e-mail; Trebuie sa configurati un server de SMTP pentru a trimite mail-uri din localhost';
	}
    ...
    if($send=='trimis') {
      ...
      <p>Va multumim pentru interesul acordat.</p>
              <p>Daca doriti, vizitati: <a href="localhost">Magazinul nostru !</a></p>';
    }
    else $re = $send;
  }
